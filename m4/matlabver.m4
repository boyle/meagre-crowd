...
[if test "${MATLAB_VERSION+set}" = set ; then
    ...
    for ax_arch in alpha glnx86 glnxa64 hp700 hpux ibm_rs sgi sol2 win32 . ; do
	...
	if test -f $ax_matlab_exec ; then
	    ...
	    # shared library.
	    ax_matlab_exec_2=`find $MATLAB/bin/$ax_arch -type f -name libmwservices\* -print 2> /dev/null | grep -v .csf`
