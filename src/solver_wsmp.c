...
void wssmp_initialize_();
void wgsmp_initialize_();
void pwssmp_initialize_();
void pwgsmp_initialize_();
...
void pwgsmp( int* N, int* IA, int* JA, int* AVALS, int* B, int* LDB, int* NRHS, int* RMISC, int* IPARM, int* DPARM );
// N: col/rows per process (on this process)
// IA: N+1 row/col ptrs into JA, if N=0, JA[0]=0
// JA: NZ col/row indices
//

void wsmp_chkmatrix( int *, int *, double *, int *, int *, int * );
void wsmp_chkvec( int *, int *, double *, int * );
void wsmp_printstats( int *, int *, double *, int *, int *, int *,
                         double *, int * );

enum wsmp_mtype { REAL_PATTERN_SYM = 1, REAL_SYM_POSDEF = 2, REAL_SYM_INDEF = -2,
...
enum wsmp_solver { SPARSE_DIRECT_SOLVER = 0, MULTIRECURSIVE_ITERATIVE_SOLVER = 1};

enum wsmp_solve_error { NO_ERROR = 0, INCONSISTENT_INPUT = -1,
                           MEM = -2, REORDERING = -3,
...
//  assert(stdout != NULL);
  ...
  // or wgsmp_intialize() if only SMP
  pwgsmp_initialize_(); // MPI
...
//  stdout = o;
  ...
  if ( error_code != NO_ERROR )
    fprintf( stderr, "error: wsmp initialization code %d\n", error_code ); // TODO decode
...
void solver_analyze_wsmp( solver_state_t* s, matrix_t* A ) {
  ...
  // TODO wsmp can handle pattern symmetric matrices that have unsymmetric data
  ...
  int error_code = 0;
  wsmp( p->PT, &MAXFCT, &MNUM, &( p->MTYPE ), &PHASE, &N,
...
void solver_evaluate_wsmp( solver_state_t* s, matrix_t* b, matrix_t* x ) {
  ...
  wsmp( p->PT, &MAXFCT, &MNUM, &( p->MTYPE ), &PHASE, N,
           ...
           &MSGLVL, B, X, &error_code, p->dparm );
  if ( error_code != NO_ERROR )
    fprintf( stderr, "error: wsmp solve code %d\n", error_code ); // TODO decode
  assert( error_code == NO_ERROR );
}
